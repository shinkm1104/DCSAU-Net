...
# Xu, Qing, et al. "DCSAU-Net: A deeper and more compact split-attention U-Net for medical image segmentation." Computers in Biology and Medicine 154 (2023): 106626.
# DCSAU-Net(2023)
# DCSAU-Net: A Deeper and More Compact Split-Attention U-Net for Medical Image Segmentation
# https://github.com/xq141839/DCSAU-Net
# the original code has error compared to paper, which is fixed by adding group=2 in rsoftmax, and make that number of blocks is [1,1,1,1]
...
class SplAtConv2d(Module):
    ...
    def __init__(self, in_channels, channels, kernel_size, stride=(1, 1), padding=(0, 0),
        ...
        self.fc1 = Conv2d(channels, inter_channels, 1, groups=2)
        if self.use_bn:
            self.bn1 = norm_layer(inter_channels)
        self.fc2 = Conv2d(inter_channels, channels*radix, 1, groups=2)
...
class PFC(nn.Module):
    def __init__(self, in_channels, channels, kernel_size=7):
        super(PFC, self).__init__()
        self.input_layer = nn.Sequential(
                    nn.Conv2d(in_channels, channels, kernel_size, padding=  kernel_size // 2),
                    # nn.Conv2d(3, channels, kernel_size=3, padding= 1),
                    nn.ReLU(inplace=True),
                    nn.BatchNorm2d(channels))
        self.depthwise = nn.Sequential(
...
# print(pytorch_model_summary.summary(DCSA_UNet_Original(1),torch.rand((1, 1, 512, 512))))
